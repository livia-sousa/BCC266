...
int procurarHD(BlocoMemoria **RAM, int endereco, int tamanho){
    ...
    fseek(HD, endereco * 16, SEEK_SET);

    for(int i = 0; i < 4; i++ ){
        fread(&aux_troca->palavra[i], sizeof(int), 1, HD);
    }
    ...
    if(posicaoVazia != -1){
        ...
        if(RAM[posTroca]->atualizado == 1){
            ...
            BlocoMemoria *words_aux = criarBlocoMem(1); 

            transfereBloco(words_aux, RAM[posTroca]);
            transfereBloco(RAM[posTroca], aux_troca);
            transfereBloco(aux_troca, words_aux);
            ...
            for(int i = 0;i < 4; i++){
                fwrite(&aux_troca->palavra[i], sizeof(int), 1, HD);
            }
